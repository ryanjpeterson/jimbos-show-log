...
// GET /concerts/:id - Single concert by ID (Used mostly for the Edit Page)
...
router.post('/create/:type', authMiddleware, async (req, res) => {
  ...
  try {
    if (type === 'concert') {
      const { date, artist, venueId, ...rest } = req.body;
...
router.put('/edit/:type/:id', authMiddleware, async (req, res) => {
  ...
  try {
    if (type === 'concert') {
      ...
      const { venueId, date, venue, ...rest } = data;
      ...
      // NEW: Remove 'id' and 'type' so they don't get sent to the DB update
      const { id, type, latitude, longitude, concerts, ...rest } = data; 
      ...
      const updated = await prisma.venue.update({
        where: { id: parseInt(req.params.id) }, // 'id' belongs here...
        data: updateData // ...but NOT inside here
      });
      return res.json(updated);
    } 
